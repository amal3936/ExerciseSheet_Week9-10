...
int main()
{
	...
	cout << " Dijhkstra " << endl;
	vector<int> shortesPath = digraph.dijkstra(0, 6, path, dist);
	cout << "Shortest way from start to goal is to ";
	for (int i = 0; i < shortesPath.size(); i++)
	{
		cout << shortesPath[i] << (i < shortesPath.size() - 1 ? " --> " : "\n");
	}
	for (int i = 0; i < 7; i++)
	{
		cout << "Distance from 0 to " << i << " is " << dist[i] << endl;
