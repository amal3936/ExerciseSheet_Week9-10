...
vector<int> Graph::dijkstra(int start, int goal, vector<int> &path, vector<int> &dist)
{
	...
	struct pair_comp
	{
		constexpr bool operator()(pair<int, int> const &a, pair<int, int> const &b) const noexcept
		{
			return a.first > b.first; // Sammenligner afstand (lavest først)
		}
	};
	// Prioriteringskø til at vælge knuden med den mindste afstand først
	priority_queue<pair<int, int>, vector<pair<int, int>>, pair_comp> q;
	vector<bool> visited(adj.size()); // Liste over besøgte knuder

	// Finder afstandene til alle knuder til uendelig og besøgte til falsk
	for (int v = 0; v < adj.size(); ++v)
	{
		dist[v] = INFINITY; // til start er afstanden uendelig
		visited[v] = false;
		path[v] = -1; // Ingen knude er endnu besøgt
	}
	dist[start] = 0;					   // Afstand til startknuden er 0
	q.push(make_pair(dist[start], start)); // Tilføj startknuden til prioriteringskøen

	//  Dijkstras algoritme køres indtil målknuden er nået eller køen er tom
	while (!q.empty())
	{
		int u = q.top().second; // Få den knude med lavest afstand fra køen
		q.pop();
		visited[u] = true; //  knuden er besøgt
		if (u == goal)
			break; // Tidlig afslutning hvis målknuden findes
		// Gennemgår alle naboknuderne til den valgte knude
		for (auto v : adj[u])
		{
			// Opdater afstanden til naboknuden, hvis den er mindre end den nuværende
			if (!visited[v] && dist[u] != INFINITY && dist[u] + weight[u][v] < dist[v])
			{
				dist[v] = dist[u] + weight[u][v]; // Opdater afstanden
				path[v] = u;					  // Opdater forældreknuden til naboknuden
				q.push(make_pair(dist[v], v));	  // Tilføj naboknuden til prioriteringskøen
			}
		}
	}
